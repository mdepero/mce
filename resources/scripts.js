...
var serverRootURL = "http://ceclnx01.cec.miamioh.edu/~deperomm/mce/serverfile.php";
...
function getSemesterList(){
  ...
  $.each(returnedData, function(index, value) {
    ...
    $('#semesters').append('<button type="button" class="btn btn-default loadStudents" onclick="loadStudentsFromSemester(\''+value+'\');">'+value+'</button><br />');
...
function displayStudentReviews( id ){
  ...
  if ($('#student_'+id).find('#studentReviewWrapper_'+id).length){
    // already queried
    $('#studentReviewWrapper_'+id).slideToggle(FAST_ANI_SPEED);
